...
router.post("/", async function (req, res, err) {
  ...
  fs.readFile("./output/package.json".toString(), "utf8", function (err, data) {
    ...
    fs.readFile("./output/src/main/resources/config/application-prod.yml".toString(), "utf8", function (err, data) {
      if (err) {
        return console.log(err);
      }
  ...
  cmdService.execShellCommand(
    "cd output/target && java -jar blog-0.0.1-SNAPSHOT.jar"
  );
  console.log("build complete!");

  res.send("Application is being deployed, Pl");
});
