...
router.post("/", async function (req, res, err) {
  ...
  console.log(
    "Modifying Package.json to remove clientblueprint as dev dependency"
  );
  ...
  // Build the Executable JAR from application-prod.yml
  ...
  fs.readFile(
    "./ouput/src/main/resources/config/application-prod.yml".toString(),
    "utf8",
    function (err, data) {
      if (err) {
        return console.log(err);
